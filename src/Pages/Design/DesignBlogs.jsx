...
const DesignBlogs = () => {
  ...
  return (
    <>
      ...
      <section className="blogPostt">
        <article className="blogContentt">
          {" "}
          <summary>
            <h3 className="thirdHeading">{section.title}</h3>
            <p className="blogsummmary">{section.explanation}</p>
          </summary>
          {Object.entries(section)
            .filter(([key]) => key.startsWith("part"))
            .map(([partKey, part], partIndex) => (
              <section key={partIndex}>
                <h3 className="secondHeading">{part.title}</h3>
                <p className="blurbParagraphh">{part.description}</p>
                <p className="blurbParagraphh">{part.description2}</p>
                <p className="blurbParagraphh">{part.description3}</p>
                {part.images && (
